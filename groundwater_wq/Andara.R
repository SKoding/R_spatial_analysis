...
andra_wq <- read.csv('Andhara_groundwq.csv') 
#dataframes for Sodium and potassium
andra.sodium<- data.frame(andra_wq$Amount.of.Sodium,andra_wq$Ground.Water.Station.Latitude,andra_wq$Ground.Water.Station.Longitude)
andra.potassium<- data.frame(andra_wq$Amount.of.Potassium,andra_wq$Ground.Water.Station.Latitude,andra_wq$Ground.Water.Station.Longitude)
#descriptive analysis
summary(andra.sodium)
summary(andra.potassium)
hist(andra.sodium$andra_wq.Amount.of.Sodium, breaks=30, col = 'green', main = 'Sodium Histogram', xlab = 'Sodium')
hist(andra.potassium$andra_wq.Amount.of.Potassium, breaks=30, col = 'blue', main = 'Potassium Histogram', xlab = 'Potassium')
...
andra.sodium['log_Na']<-log(andra.sodium$andra_wq.Amount.of.Sodium)
andra.potassium['log_K']<-log(andra.potassium$andra_wq.Amount.of.Potassium)

hist(andra.sodium$log_Na, breaks=30, col = 'green', main = 'LogNa Histogram', xlab = 'Sodium')
hist(andra.potassium$log_K, breaks=30, col = 'blue', main = 'LogK Histogram', xlab = 'Potassium')
...
#clipping to AOI
andra.sodium <- raster::crop(andra.sodium,andra)
andra.potassium <- raster::crop(andra.potassium,andra)
...
andra.sodium %>% as.data.frame %>% 
  ...
  ggtitle("Andra Pradesh Sodium Concentrations", subtitle = 'Ground water amount of Sodium in andra Pradesh state') + 
...
andra.potassium %>% as.data.frame %>% 
  ggplot() + 
  geom_point(aes(longitude,latitude,size=potassium),color = rainbow(3486), alpha=3/4) + 
  ggtitle("Andra Pradesh Potassium Concentrations", subtitle = 'Ground water amount of Potassium in andra Pradesh state') + 
  theme_bw() + geom_sf(data = andra,fill=NA) + 
  xlab('Longitude (Degrees)') + ylab('Latitude (Degrees)') + 
...
######Thin Plate Spline
andra.sodium.tps<- data.frame(andra_wq$Amount.of.Sodium,andra_wq$Ground.Water.Station.Latitude,andra_wq$Ground.Water.Station.Longitude)
andra.potassium.tps<- data.frame(andra_wq$Amount.of.Potassium,andra_wq$Ground.Water.Station.Latitude,andra_wq$Ground.Water.Station.Longitude)

names(andra.sodium.tps) <- c('sodium','latitude','longitude')
names(andra.potassium.tps) <- c('potassium','latitude','longitude')
#sodium
Na_TPS <- Tps( x = as.matrix(andra.sodium.tps[, c('longitude','latitude')]),Y = andra.sodium.tps$sodium)
...
K.fit <- fit.variogram(K.vgm,vgm('Sph'), fit.method = 6)
