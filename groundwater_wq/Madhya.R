...
setwd("~/Documents/Upwork/Spatial_analysis/groundwater_wq")
#data reading
madhya <- st_read('shps/MadhyaPradesh.shp')
madhya_wq <- read.csv('Madhya_groundwq.csv') 
#dataframes for Sodium and potassium
madhya.sodium<- data.frame(madhya_wq$Amount.of.Sodium,madhya_wq$Ground.Water.Station.Latitude,madhya_wq$Ground.Water.Station.Longitude)
madhya.potassium<- data.frame(madhya_wq$Amount.of.Potassium,madhya_wq$Ground.Water.Station.Latitude,madhya_wq$Ground.Water.Station.Longitude)
#descriptive analysis
summary(madhya.sodium)
summary(madhya.potassium)
hist(madhya.sodium$madhya_wq.Amount.of.Sodium, breaks=30, col = 'green', main = 'Sodium Histogram', xlab = 'Sodium')
hist(madhya.potassium$madhya_wq.Amount.of.Potassium, breaks=30, col = 'blue', main = 'Potassium Histogram', xlab = 'Potassium')
...
madhya.sodium['log_Na']<-log(madhya.sodium$madhya_wq.Amount.of.Sodium)
madhya.potassium['log_K']<-log(madhya.potassium$madhya_wq.Amount.of.Potassium)

hist(madhya.sodium$log_Na, breaks=30, col = 'green', main = 'LogNa Histogram', xlab = 'Sodium')
hist(madhya.potassium$log_K, breaks=30, col = 'blue', main = 'LogK Histogram', xlab = 'Potassium')
...
#clipping to AOI
madhya.sodium <- raster::crop(madhya.sodium,madhya)
madhya.potassium <- raster::crop(madhya.potassium,madhya)
...
madhya.sodium %>% as.data.frame %>% 
  ...
  ggtitle("madhya Pradesh Sodium Concentrations", subtitle = 'Ground water amount of Sodium in madhya Pradesh state') + 
...
madhya.potassium %>% as.data.frame %>% 
  ggplot() + 
  geom_point(aes(longitude,latitude,size=potassium),color = rainbow(7392), alpha=3/4) + 
  ggtitle("madhya Pradesh Potassium Concentrations", subtitle = 'Ground water amount of Potassium in madhya Pradesh state') + 
  theme_bw() + geom_sf(data = madhya,fill=NA) + 
  xlab('Longitude (Degrees)') + ylab('Latitude (Degrees)') + 
...
##############Nearest Neighbor
#Sodium
Na.NN <- idw(formula = sodium ~ 1, madhya.sodium,grd,nmax = 1, nmin = 1, debug.level = -1)
plot(Na.NN)
...
######Thin Plate Spline
madhya.sodium.tps<- data.frame(madhya_wq$Amount.of.Sodium,madhya_wq$Ground.Water.Station.Latitude,madhya_wq$Ground.Water.Station.Longitude)
madhya.potassium.tps<- data.frame(madhya_wq$Amount.of.Potassium,madhya_wq$Ground.Water.Station.Latitude,madhya_wq$Ground.Water.Station.Longitude)

names(madhya.sodium.tps) <- c('sodium','latitude','longitude')
names(madhya.potassium.tps) <- c('potassium','latitude','longitude')
#sodium
Na_TPS <- Tps( x = as.matrix(madhya.sodium.tps[, c('longitude','latitude')]),Y = madhya.sodium.tps$sodium)
...
###KRIGING
Na.vgm <- variogram(sodium~1, madhya.sodium, cutoff=6) #creating an Empirical variogram
K.vgm <- variogram(potassium~1, madhya.potassium, cutoff=6)
plot(Na.vgm)
plot(K.vgm)

#create a single model to get parameter values for vgm e.g. psill, nugget, range...
Na.fit <- fit.variogram(Na.vgm,vgm(3000,'Sph',1), fit.method = 2) #fit vgm
K.fit <- fit.variogram(K.vgm,vgm(110,'Sph',1), fit.method = 2)
...
##CROSS VALIDATION
Na.gst <- gstat(formula = sodium~1, data = Na.clean,model =  Na.fit)
cv <- gstat.cv(Na.gst)
spplot(cv)
